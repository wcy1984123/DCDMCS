...
/**
 * Adapter for hidden markov model
 * From: http://doc.gold.ac.uk/~mas02mg/software/hmmweka/#download
 */

public class HMMAdapter extends RandomizableClassifier implements OptionHandler, MultiInstanceCapabilitiesHandler {

    private static final Logger LOGGER = Logger.getLogger(HMMAdapter.class.getName());

    //---------------------------------- Variable Declaration ----------------------------------//

    /**
     * A probability too small exception class
     */
    ...
    protected HMMEstimatorAdapter estimators[];
     ...
     * @return
     */
    public HMMEstimatorAdapter[] getEstimators() {
        return estimators;
     ...
     */
    public double[][] getStateTransitionMatrix(HMMEstimatorAdapter estimator) {
        return estimator.getStateTransitionMatrix();
    }
     ...
     * @return state transition matrix
     */
    public double[][] getOutputTransitionMatrix(HMMEstimatorAdapter estimator) {
        return estimator.getOutputTransitionMatrix();
    }
     ...
     */
    public double[] getInitialStateTransitionMatrix(DiscreteHMMEstimatorAdapter estimator) {
        return estimator.getInitialStateTransitionMatrix();
    }



    /**
     * Get the index of the data attribute containing the sequence data
     */
    ...
    /**
     * Get whether the HMM state probabilities are randomly initialized.
    ...
    /**
     * Set whether the HMM state probabilities are randomly initialized.
     * If this is false (default) the states are set using clustering on the dataset.
     * @param randomStateInitializers if true the HMM state probabilities will be initialized randomly;
     *                                if false they will be initialized using clustering
     */
    ...
    /**
     * Get whether the covariance matrices of guassian HMMs are tied.
    ...
    /**
     * Set whether the covariance matrices of guassian HMMs are tied.
     * Tied covariances are the same for all states of the HMM.
     * Using tied covariances makes it possible to learn models from less
     * data but restricts the class of models that can be learned.
     *
    ...
    /**
     * Get whether the HMM has a Left-Right state structure
    ...
    /**
     * Set whether the HMM has a Left-Right state structure
     * In a <em>left-right</em> HMM the sequences of states are always
     * passed through in a fixed order (possibly missing
     * some states). The alternative is an <em>ergodic model</em> (default) in which
     * arbitrary transitions can be made.
    ...
    /**
     * Get the dimensionality of the HMM outputs (observations)
     */
    ...
    /**
     * Get whether the outputs are numeric
    ...
    protected void setNumeric(boolean Numeric) {
        m_Numeric = Numeric;
        if (m_Numeric) setIterationCutoff(0.0001);
        else setIterationCutoff(0.01);
    }

    /**
     * Get the cut off value of terminating the EM iteration.
    ...
    /**
     * Set the cut off value of terminating the EM iteration.
     * The EM iteration will stop when the relative change in likelihood
     * between two subsequent iterations falls below this cutoff
     *
    ...
    /**
     * Get the number of classes for the HMM classifier.
     * The value is set automatically from the data
     */
    public int getNumClasses() {
        if(estimators == null) return 0;
        else return estimators.length;
    }
    ...
    /**
     * Set the number of hidden states in the HMM.
    ...
    /**
     * Get the number distinct output (observation) values for nominal data
    ...
    /**
     * Set the number distinct output (observation) values for nominal data
     * This is set automatically from data.
     * @param numOutputs
     */
    ...
    /**
     * Set a step 0 state/output probability
     *
    ...
    /**
     * Set a state/output probability
     *
     * @param classId the class for which we are setting the probability
    ...
    protected double likelihoodFromScales(double scales[])
    {
        ...
        for (int i = 0; i < scales.length; i++) {
            if (Math.abs((scales[i])) > 1.0E-32) lik += Math.log(scales[i]);
            else lik += Math.log(1.0E-32);
        }

        return lik;
    }

    protected double [] forward(HMMEstimatorAdapter hmm, Instances sequence, double alpha[][]) throws Exception
    {
     ...
     */
    protected double forward(HMMEstimatorAdapter hmm, Instances sequence) throws Exception
    {
    ...
    protected double [] forwardBackward(HMMEstimatorAdapter hmm, Instances sequence, double alpha[][], double beta[][]) throws Exception
    {
        // do the forward pass
        double scales [] =  forward(hmm, sequence, alpha);
    ...
    protected double forwardBackward(HMMEstimatorAdapter hmm, Instances sequence) throws Exception
    {
        double alpha[][] = new double[sequence.numInstances()][m_NumStates];
    ...
    protected double EMStep(Instances data) throws Exception
    {
        ...
        HMMEstimatorAdapter newEstimators[] = new HMMEstimatorAdapter[data.numClasses()];;
...
//                newEstimators[i]= est;
            ...
            {
                newEstimators[i]=new DiscreteHMMEstimatorAdapter(getNumStates(), getNumOutputs(), false);
            }
        }
        ...
        for (int i = 0; i < data.numInstances(); i++)
        {
            ...
            HMMEstimatorAdapter hmm = estimators[classNum];
...
//    protected void initGaussianOutputProbsRandom(int numClasses, DoubleVector outputMeans[][], weka.core.matrix.Matrix outputVars[][])
//    {
//        if (outputMeans == null)
//        {
...
//        if (outputVars == null)
//        {
//            outputVars = new weka.core.matrix.Matrix[numClasses][getNumStates()];
//            for (int i = 0; i < numClasses; i++)
//                for (int j = 0; j < getNumStates(); j++)
//                {
//                    outputVars[i][j] = weka.core.matrix.Matrix.identity(getOutputDimension(), getOutputDimension());
...
//    }
//
//    protected void initGaussianOutputProbsAllData(int numClasses, Instances data, DoubleVector outputMeans[][], weka.core.matrix.Matrix outputVars[][]) throws Exception
//    {
...
//    }
//
//    protected void initGaussianOutputProbsCluster(int numClasses, Instances data, DoubleVector outputMeans[][], weka.core.matrix.Matrix outputVars[][]) throws Exception {
//
//
...
//                    weka.core.matrix.Matrix sigma = new weka.core.matrix.Matrix(n,n, 0.0);
//                    for(int a = 0; a < n; a++) {
...
//    }
    ...
    /**
     * Initialize estimators for univariate discrete distribution
     * @param numClasses number of classes
     * @param state0Probs probability of state 0
     * @param stateProbs probability of current state given the previous state
     * @param outputProbs output probabilities
     * @throws Exception
     */
    ...
    {
        estimators = new HMMEstimatorAdapter[numClasses];
        ...
        for (int i = 0; i < numClasses; i++) {
            estimators[i]=new DiscreteHMMEstimatorAdapter(getNumStates(), getNumOutputs(), false);
            for (int s = 0; s < getNumStates(); s++)
                for (int o = 0; o < getNumOutputs(); o++)
                {
...
//    /**
//     * Initialize estimator of multivariate normal distribution
//     * @param numClasses number of classes
//     * @param state0Probs probability of state 0
//     * @param stateProbs probability of current state given the previous state
//     * @param outputMeans output means
//     * @param outputVars output variance
//     * @param data input data
//     * @throws Exception
//     */
//    protected void initEstimatorsMultivariateNormal(int numClasses, double state0Probs[][], double stateProbs[][][], DoubleVector outputMeans[][], weka.core.matrix.Matrix outputVars[][], Instances data) throws Exception {
...
//    }
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        if (estimators == null) initEstimators(data.numClasses(), data);

        // Bear comment it out
        //for(int i = 0; i < estimators.length; i++) System.out.println(i + " " + estimators[i]);
...
//					|| Math.abs(lik) < getIterationCutoff())
    ...
    public Instances sample(int numseqs, int length, Random generator)
    {
        ...
        for (int i = 0; i < estimators.length; i++) classNames.add("class_"+i);
        attrs.add(new Attribute("class", classNames));

        ArrayList<Attribute> seqAttrs = new ArrayList<Attribute>();

        // Determine if it is continuous HMM or discrete HMM
        if(isNumeric()) {
            ...
            for(int i = 0; i < getNumOutputs(); i++) outputs.add("output_"+i);
        ...
        for (int seq=0; seq<numseqs; seq++) {
            ...
            HMMEstimatorAdapter est = estimators[classId];
    ...
    public static void main(String [] argv) {
        runClassifier(new HMMAdapter(), argv);
    }
}
